<template>
  <div class="undidhome">
    <p class="persontop" style="margin-bottom:20px;">我的代办任务</p>
      ...
      <el-table :data="undidData" border style="width: 100%" >
        ...
        <el-table-column label="标题">
          <template slot-scope="scope"> 
            <router-link :to="{path:'/project/detailapprove?id='+scope.row.id+'&InId='+scope.row.processInstanceId+'&flowType='+scope.row.flowType}">{{scope.row.subject}}</router-link>              
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button type="text" icon="el-icon-info">详情</el-button> 
             ...
             <router-link to> 
              <el-button type="text"  style="color:#F56C6C" icon="el-icon-delete-solid" v-if="scope.row.userId == 103" @click="deletedata(scope.row.processInstanceId,scope.row.flowType)">删除</el-button> 
            </router-link>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getall(){
      ...
      this.$http.get(personapi.inquery,{params:data}).then(res=>{
        console.log(res);
        ...
        // console.log(error)
        // console.log(error)
        // if(error == "Error: Network Error at createError (createError.js?16d0:16) at XMLHttpRequest.handleError (xhr.js?ec6c:87)"){
        //   console.log(1)
        // }
      })
    },
    deletedata(id,type){
       this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.$http.get(personapi.gorflowDelete,{params:{processInstanceId:id,flowType:type}}).then(res=>{
