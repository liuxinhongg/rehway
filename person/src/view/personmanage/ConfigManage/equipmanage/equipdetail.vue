<template>
  <div class="accredetail">
    <p class="toptext">设备表单</p>
    <div class="panel">
      <el-form class="form-horizontal" ref="addequip"  @submit.native.prevent :model="addequip" :rules="equiprule">
        <table border="1" class="tableinsert" style="width:95%;">
          <tr>
              <td class="tdleft">设备名称：</td>
              <td>
                <el-form-item label="" prop="name">
                  <el-input v-model.trim="addequip.name" style="width:280px;" placeholder="请输入设备名称"></el-input>
                </el-form-item>
              </td>
              <td class="tdleft">设备序列号：</td>
              <td>
                <el-form-item prop="serial" >
                  <el-input v-model.trim="addequip.serial" style="width:280px;" disabled></el-input>
                </el-form-item>
              </td>
          </tr>
          <tr>
              <td class="tdleft">设备IP：</td>
              <td>
                <el-form-item prop="ip" >
                  <el-input v-model.trim="addequip.ip" style="width:280px;"  placeholder="请输入设备IP"></el-input>
                </el-form-item>
              </td>
              <td class="tdleft">设备密码：</td>
              <td>
                <el-form-item prop="password">
                    <el-input v-model="addequip.password" type="password" style="width:280px;" disabled ></el-input>
                    <el-button @click="equipVisible = true" size="small">设置密码</el-button>
                </el-form-item>
              </td>
          </tr>
          <tr>
      ...
      <el-dialog title="设置设备密码" :visible.sync="equipVisible" width="30%" class="modleerror"  @submit.native.prevent center>
        <el-form :model="equipform" ref="equipform" :rules="quiprules" label-width="70px" style="width:80%;margin:auto;">
          <el-form-item prop="oldpass" style="text-align:center;">
            <span>旧密码：</span>
            <el-input v-model="equipform.oldpass" autocomplete="off" style="width:200px;" placeholder="请输入旧密码"></el-input>
          </el-form-item>
          <el-form-item prop="newpass" style="text-align:center;">
            <span>新密码：</span>
            <el-input v-model="equipform.newpass" autocomplete="off" style="width:200px;" placeholder="请输入新密码"></el-input>
          </el-form-item>
          <el-form-item  style="text-align:center;">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      equiprule:{
        ...
        {pattern:/^[\u4e00-\u9fa5_a-zA-Z]{2,10}$/,message:'请输入正确的设备名称'}
        ],
          ...
          if(that.equipId==0){
            ...
            that.$http.get('/api/attendance_machine/attend_machine_insert_info',{params:data}).then(res=>{
              ...
              if(res.data.result='success'){
                let data = {
                  callbackUrl:'http://'+this.equipgetIP+'/check/insert',
                  pass:this.attequippass
                }
                this.$http.get('/equip/setIdentifyCallBack',{params:data}).then(datal=>{
                  console.log(datal)
                })
        ...
        if (valid) {
          this.$http.post('/equip/setPassWord?oldPass='+this.equipform.oldpass+'&newPass='+this.equipform.newpass,
            ...
            if(res.data.data.length>9){
              ...
              // this.attequippass=this.addequip.password;
  ...
  created() {
    ...
    this.$http.get('/api/get/ipAddress').then(res=>{
      console.log(res);
      this.equipgetIP=res.data;
    })
  },
  mounted() {
...
.modleerror .el-form-item__error{
  left: 137px;
}
</style>
