...
<template>
  <div class="attendlist">
    ...
    <el-scrollbar style="height:100%;">
      ...
      <div id="subOutputRank-print" ref="examAnswerContent">
        <el-table :data="unfinishData" class="unfindTable" border>
          ...
          <el-table-column prop="workerId" label="工人编号" sortable></el-table-column>
          ...
          <el-table-column label="上班打卡时间" prop="date" column-key="attendTime">
            <template slot-scope="scope">
              <p>{{scope.row.clockTime | formatDate}}</p>
            </template>
          </el-table-column>
          <el-table-column label="下班班打卡时间" prop="date" column-key="attendTime">
            <template slot-scope="scope">
              <p>{{scope.row.closeTime | formatDate}}</p>
            </template>
          </el-table-column>
    ...
    <el-dialog title="考勤时间配置" :visible.sync="kqVisible" width="35%" center>
      <el-form :model="timeform" label-width="120px" :rules="timeconfig">
        <el-form-item label="开始时间：" style="margin-left:40px;" prop="startTime">
            <el-time-select placeholder="起始时间" v-model="timeform.startTime"
              :picker-options="{ start: '08:30', step: '00:15', end: '18:30' }">
            </el-time-select>
        </el-form-item>
        <el-form-item label="结束时间：" style="margin-left:40px;" prop="endTime">
            <el-time-select placeholder="结束时间" v-model="timeform.endTime"
                :picker-options="{ start: '08:30',  step: '00:15', end: '18:30', minTime: timeform.startTime }">
            </el-time-select>
        </el-form-item>
        <el-form-item label="误差时间：" style="margin-left:40px;" prop="TimeLength">
            <el-input placeholder="误差时间" v-model="timeform.TimeLength" :mim="1" style="width:220px;"> </el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      timeconfig:{
        ...
        TimeLength:[ { required: true, message: '请输入时间误差', trigger: 'blur' },
        {pattern:/^[0-9]{1,2}$/,message:'时间误差设备超时'}
        ],
      },
      form: {
        date1: "",
  ...
  methods: {
    ...
    suretime(){
      let data={
        clockTime:this.startTime,
        closeTime:this.endTime,
        timeLength:this.TimeLength
      }
      this.$http.get('/api/clockConfig/insert',{params:data}).then(res=>{
        if(res.data.result=="success"){
          this.$message.success('设置考勤时间成功~');
        }
      })
    },
    configkq(){
    ...
    getAlldata(){
      ...
      that.$http.get("/api/attendance/log/selectBy", {params: data}).then(res => {
          console.log(res);
  ...
  //   websocketatten(){
  //     // var ws = new WebSocket('ws://192.168.3.110:8081/websocket');
  ...
  //       // console.log('from server: ' + e.data.JSON.parse);
  //       console.log(JSON.parse(e.data))
  //       let  Ipaddress=JSON.parse(e.data).ipAddress;
  //       let  sessionId=JSON.parse(e.data).sessionId;
  //       console.log(Ipaddress+'sssss'+sessionId)
  //       // let data={
  //       //   workerId:e.data
  //       // }
  //       // this.$http.get('/api/socket/save')
  //     };
      
  //     this.ws.onclose=function(e){
  //       console.log("关闭 (" + e.code + ")");
  //     }
  //     //连接发生错误的回调方法
