...
<template>
  <div>
    <p class="toptext">公告列表</p>
    <el-form :inline="true" :model="newsform" class="searchTop" @submit.native.prevent>
      <el-form-item>
        <span>标题：</span> 
       <el-input placeholder="请输入标题" v-model.trim="newsform.title" style="width:200px;"></el-input>
      </el-form-item>
      <el-form-item>
        <span>发布者：</span> 
        <el-input placeholder="请输入发布者" v-model.trim="newsform.issuer" style="width:200px;"></el-input>
      </el-form-item>
      <el-form-item>
        <span>发布时间：</span>
      ...
      <el-form-item style="float:right;">
          <el-button style="margin:-13px 0px"  @click='addnew()' type="success">+添加</el-button>
    ...
    <el-table :data="adverData" border style="width: 90%;margin:0 auto;">
      ...
      <el-table-column label="新闻附件">
        <template slot-scope="scope" >
          <!-- <div> -->
            <a v-for="(item,index) in scope.row.file" :key="index" :href="'/api/took/user_get_photo?fileName='+item.location">{{ item.fileName =='' ? '暂无附件' : item.fileName }}</a>
          <!-- </div> -->
        </template>
      </el-table-column>
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button type="text"  style="color:#F56C6C" icon="el-icon-delete-solid"  @click="deletehanle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="block">
  ...
  methods: {
    deletehanle(id){
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.$http.get(personmanage.deletenotice,{ params:{id:id} }).then(res=>{this.getAll();})
    ...
    addnew(){
      console.log(11);
      this.$router.push('/project/annadd');
    },
    search(){this.getAll()},
    ...
    getAll(){
      ...
      this.$http.get(personmanage.noticelist,{ params:data }).then(res=>{
        console.log(res);
