    ...
    <el-scrollbar style="height:100%;overflow-x: hidden;">
      ...
      <el-dialog  :visible.sync="dialogVisible"  width="40%"  :before-close="handleClose">
            ...
            <tr>
              ...
              <td>
                <el-form-item prop="ctid">
                  <el-select v-model="jsform.ctid" @change="htdetails">
                      <el-option
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="submitselect('jsform')">生成结算</el-button>
        </span>
      </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
              ...
              this.$http.get(material.mcontractdetailapprToAct,{params:{provId:provId}}).then(res=>{
                if(res.data.result == 'success'){
                  this.$http.get(material.mactMonthcreateActMonth).then(res=>{
                      console.log(res);
                      this.jsform.ctId = res.data.actId
    ...
    // 合同
    htdetails(){
      this.$http.get(material.mcontractselect,{params:{ctid:this.jsform.ctid}}).then(res=>{
        console.log(res);
