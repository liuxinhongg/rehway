    ...
    <div class="integral">
        <el-scrollbar style="height:100%;">    
            <el-form :model="integral" :inline="true" label-width="120px;" @submit.native.prevent>
                <el-form-item label="区域配置名称：">
                    <el-input v-model="integral.name"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" native-type="submit" icon="el-icon-search" @click="seach">搜索</el-button>
                </el-form-item>
                <el-form-item style="float:right;">
                    <el-button @click="addintegral">+ 新建</el-button>
                </el-form-item>
            </el-form>
            <el-table :data="integraldata" style="width: 100%" border>
                <el-table-column prop="id" label="编号" width="150"></el-table-column>
                <el-table-column prop="locName" label="名称"></el-table-column>
                <el-table-column prop="sumintegral" label="区域图">
                    <template slot-scope="scope">
                        <img :src="'/api/took/user_get_photo?fileName='+scope.row.locPic" alt="" srcset="" style="width:80px;height:50px;">
                    </template>
                </el-table-column>
            ...
            <el-form :model="localform" ref="localform">
                <table border="1" class="tableinsert" style="width:100%;">
                    <tr>
                        ...
                        <td>
                            <el-form-item prop="locName" :rules="[{ required: true, message: '请输入区域名称', trigger: 'blur' }]">
                                <el-input v-model="localform.locName" style="width:280px"></el-input>
                            </el-form-item>
                        </td>
    ...
    methods:{
        ...
        submitForm(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    sublocalform.append('filepath',this.localform.filepath)
                    this.$http.post('/api/location/insert',sublocalform,{ headers: { 'Content-Type': 'application/x-www-form-urlencoded' }}).then(res=>{
                        console.log(res);
        ...
        deletehandle(id){
            this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
                ...
                this.$http.post('/api/location/delete?id='+id).then(res=>{
                    if(res.data.result='success'){
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        localdata(){
            ...
            this.$http.get('/api/location/selectBytitle',{params:data}).then(res=>{
