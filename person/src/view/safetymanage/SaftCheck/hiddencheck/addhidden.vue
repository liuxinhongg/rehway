            ...
            <el-form :model="checkform" :rules="useryz" @submit.native.prevent  ref="checkform">
                <table border="1" class="tableinsert">
                    ...
                    <tr>
                        ...
                        <td>
                            <el-form-item prop="riskScore">
                                <el-input v-model.trim="checkform.riskScore" style="width:280px" placeholder="请输入扣除分数"></el-input>
                            </el-form-item>
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <el-form-item >
                                <el-input v-model.trim="checkform.planDest" type="textarea" :rows="3" maxlength="300" placeholder="内容不能超过300个字符"></el-input>
                            </el-form-item>
                        </td>
                    </tr>
                </table>
...
export default {
    ...
    data(){
        return{
            ...
            useryz:{
                ...
                riskScore:[{required: true,message: "请输入扣除分数",trigger: "blur"},{pattern:/^[1-9]{1}[0-2]{0,1}$/,message:'分数最大12'}],         
            },
            cheackId:0,
        }
    },
    methods:{
        ...
        hiddensubmit(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    if(this.cheackId==0){
                        this.$http.post(security.riskchkinsert,cheakhiddenform,{ headers: { 'Content-Type': 'application/x-www-form-urlencoded' }}).then(res=>{
                            console.log(res);
                        ...
                        if(this.checkform.riskState==''||this.checkform.riskState==null){
                            ...
                            this.$http.post(security.riskchkupdate,cheakhiddenform).then(res=>{
                                console.log(res);
                                if(res.data.result=='success'){
        ...
        // 隐患区域
        zonedata(){
            this.$http.post(security.locationselectAll).then(res=>{
                console.log(res);
                this.zoneoption=res.data.list;
            })
        },
        //检查类型
        checkdata(){
            this.$http.post(dictapi.ristverify).then(res=>{
                console.log(res);
        ...
        //隐患性质
        naturepedata(){
            this.$http.post(dictapi.ristdanger).then(res=>{
                console.log(res);
    ...
    mounted(){
        ...
        if(this.cheackId!=0){
            this.$http.post(security.riskchkselectByID,{params:{id:this.cheackId}}).then(res=>{
                console.log(res);
