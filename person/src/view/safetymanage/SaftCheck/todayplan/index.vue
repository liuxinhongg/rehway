<template>
  <div>
    ...
    <el-table ref="multipleTable" :data="adverData" tooltip-effect="dark" border style="width: 90%;margin:auto;">
      <el-table-column label="计划编号" type="index" width="120"></el-table-column>
      ...
      <el-table-column label="计划检查内容" width="200">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    // 添加计划
    addfile(){
      this.$router.push({path:'/saftmanage/saftaddtoday?id=0'});
    },
    // 根据条件搜索
    plansearch() {
      this.dataAll();
    ...
    // 详情
    modify(id){
      this.$router.push({path:'/saftmanage/saftaddtoday?id='+id});
    },
     uploadperson(){
