<template>
  <div class="safteducate">
    <p class="toptext">安全教育列表</p>
    <el-form :model="sousou" class="searchfrom" :inline="true" @submit.native.prevent>
      <el-form-item>
        <span>安全教育标题：</span><el-input v-model="sousou.title" placeholder="请输入标题" style="width:220px;"></el-input>
      </el-form-item>
      <el-form-item >
        <span>安全教育类型：</span>
          <el-select v-model="sousou.eduCode" style="width:220px">
...
export default {
  ...
  methods: {
    ...
    dataAll(){
      ...
      this.$http.get(security.riskeduselectBy,{params:data}).then(res=>{
        console.log(res);
  ...
  mounted(){
    this.dataAll();
    this.educateType();
    this.$http.get('/zh?id=2068').then(res=>{
      console.log(res);
    })
  }
};
