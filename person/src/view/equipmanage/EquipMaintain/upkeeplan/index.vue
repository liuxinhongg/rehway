  ...
  <div class="qualitycheck">
    <el-scrollbar style="height:100%;">
      ...
      <el-table ref="multipleTable" :data="hiddenpageData" tooltip-effect="dark" border style="width: 90%;margin:auto;">
        ...
        <el-table-column label="保养名称" prop="maintainName" sortable ></el-table-column>
        <el-table-column label="保养周期(n天/1次)" prop="chkCircle" sortable  width="200"></el-table-column>
        <el-table-column label="责任人" prop="chkManName" ></el-table-column>
        <el-table-column label="状态">
            <template slot-scope="scope">
                {{scope.row.state == 0 ?'审核中' : scope.row.state == 1 ? '审核未通过' : '审核通过'}}
            </template>  
        </el-table-column>        
        ...
        <el-table-column label="保养检查内容">
          <template slot-scope="scope">
            <el-tooltip trigger="hover" placement="top">
              <div slot="content" v-html="scope.row.chkCont"></div>
              <div class="boxInput" v-html="scope.row.chkCont" ></div>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        <el-table-column label="审核意见">
          <template slot-scope="scope">
            <el-tooltip trigger="hover" placement="top">
              <div slot="content">{{scope.row.audit}}</div>
              <div class="boxInput" >{{scope.row.audit}}</div>
            </el-tooltip>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    dataAll(){
      ...
      this.$http.get(equipmanage.devmantplanselectBy,{params:data}).then(res=>{
        console.log(res);
    ...
    // 新增
    addfile(){
      this.$router.push({path:'/equipmanage/doupkeeplan?id=0'});
    },
    // 表格中的修改
    modifyfile(id){
      this.$router.push({path:'/equipmanage/doupkeeplan?id='+id});
    },
    sousoubtn() {
      this.dataAll();
    },
